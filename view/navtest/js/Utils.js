...
function getTime(isStr) {
    ...
    let date = new Date();
    let n = temp(date.getFullYear()), y = temp(date.getMonth() + 1), r = temp(date.getDate()), s = temp(date.getHours()), f = temp(date.getMinutes()), m = temp(date.getSeconds());
    if (isStr) {
        let Timestr = n + "年" + y + "月" + r + "日 " + s + ":" + f + ":" + m;
        return Timestr;
    } else {
        return [n, y, r, s, f, m];
    }
}
// 调用语法:    getTime(Boolean: isStr);
// 返回值:  字符串 || 数组
//      字符串: 2024年10月22日 21:41:47
//      数组: [ '2024', '10', '22', '21', '42', '05' ]
// 使用方法:
//      getTime(); || getTime(true);
...
// 调用语法:    showForm(Object: form, Object: obj);
// 返回值:   无返回值;
//
// 使用方法:
//.     obj属性名与表单字段name属性值相同
//      let formObj = document.querySelector("form");
//      let obj = {name:"张三",password:"123"};
...
// 调用语法:    ajax(Object: obj);
// 返回值:       无返回值;
//
// 使用方法:
//.     假如请求地址为http://www.example.com/api
//      ajax({
//          type: "POST",
